...
function supp_s08_decode_APz(arrayInput)

% This script takes the scalp-EEG waveform from electrode APz and runs
...
timeWindow = [0,500]; % time window of interest

useThisName = ['_AT_APZ_']; %use savename relevant to above coords
...
for thisCVRep = 1:nCVReps
    ...
    loadnameFaces = ['NEW_ALL_APZ_faces_TrialData_' num2str(PName) '_All_Phases.mat']; %all data from APz for this participant
    loadnameRandom = ['NEW_ALL_APZ_random_TrialData_' num2str(PName) '_All_Phases.mat']

    savename_Results = ['results_APZ_decode_' (PName) '_' (thisPhase) '_CVmethod_10REPS_' ...
