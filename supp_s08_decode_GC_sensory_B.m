...
function supp_s08_decode_GC_sensory_B(arrayInput)
...
possHigherCoords = {'L_FFA','R_FFA'}%,higher level ROIs
possLowerCoords = {'L_Occ','R_Occ'}; % lower level ROIs
...
trainProp = '0.7'; % this training proportion
timeWindow = [0,500]; %this time window

useThisName = ['_ALL_FFA_ALL_OCC_']; %use savename relevant to above coords
...
for thisCVRep = 1:nCVReps
    ...
    for higherIdx = 1:length(possHigherCoords)

        useHigherCoord = possHigherCoords{higherIdx};

        for lowerIdx = 1:length(possLowerCoords)

            ROIcount = ROIcount + 1;
            useLowerCoord = possLowerCoords{lowerIdx};

            loadname =  ['GC_rmline_' (PName) '_' (thisPhase) '_CVmethod_thisRep' num2str(thisCVRep) ...
                '_NoFilt_wBC_zscore=_1_bw_' num2str(useHigherCoord) ...
                '_and_' num2str(useLowerCoord) '_by_TstatOverTime_usingTrainProp_' num2str(trainProp) '_and_' ...
                num2str(timeWindow(1)) '_to_' num2str(timeWindow(2)) 'ms.mat'];

            savename_Results = ['results_GC_toOcc_BackFrom_FFA_rmline_' (PName) '_' (thisPhase) '_CVmethod_10REPS_GC_' num2str(maxFreq) 'Hz_MultiROI_' ...
            ...
            %% Load Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            load([loadname],'allGC_from_PFC_TRAIN','allGC_from_PFC_TEST','freq_NP'); %load face data
            ...
            all_Train_GC_FROM_PFC_faces(:,:,ROIcount) = squeeze(allGC_from_PFC_TRAIN(1,:,1:freqBandEnd));
            all_Test_GC_FROM_PFC_faces(:,:,ROIcount) = squeeze(allGC_from_PFC_TEST(1,:,1:freqBandEnd));

            %% RANDOM: SQUEEZE DATA INTO REQUIRED FORMAT
            all_Train_GC_FROM_PFC_rand(:,:,ROIcount) = squeeze(allGC_from_PFC_TRAIN(2,:,1:freqBandEnd));
            all_Test_GC_FROM_PFC_rand(:,:,ROIcount) = squeeze(allGC_from_PFC_TEST(2,:,1:freqBandEnd));


        end
    end

    all_FACES_Sensory_to_PFC_allReps(thisCVRep,:,:,:) = [all_Train_GC_FROM_PFC_faces;all_Test_GC_FROM_PFC_faces];
    all_RANDOM_Sensory_to_PFC_allReps(thisCVRep,:,:,:) = [all_Train_GC_FROM_PFC_rand;all_Test_GC_FROM_PFC_rand];


    %% ORGANISE THE DATA ONCE ALL GATHERED
    all_Train_Face = reshape(all_Train_GC_FROM_PFC_faces,[size(all_Train_GC_FROM_PFC_faces,1),...
        size(all_Train_GC_FROM_PFC_faces,2)*size(all_Train_GC_FROM_PFC_faces,3)]);
    all_Test_Face = reshape(all_Test_GC_FROM_PFC_faces,[size(all_Test_GC_FROM_PFC_faces,1),...
        size(all_Test_GC_FROM_PFC_faces,2)*size(all_Test_GC_FROM_PFC_faces,3)]);

    all_Train_Rand = reshape(all_Train_GC_FROM_PFC_rand,[size(all_Train_GC_FROM_PFC_rand,1),...
        size(all_Train_GC_FROM_PFC_rand,2)*size(all_Train_GC_FROM_PFC_rand,3)]);
    all_Test_Rand = reshape(all_Test_GC_FROM_PFC_rand,[size(all_Test_GC_FROM_PFC_rand,1),...
        size(all_Test_GC_FROM_PFC_rand,2)*size(all_Test_GC_FROM_PFC_rand,3)]);
    ...
    clear classLabel_toPFC_test allTest_Sensory_to_PFC model allTrain_Sensory_to_PFC all_Train_GC_FROM_PFC_faces ...
        all_Test_GC_FROM_PFC_faces all_Train_GC_TO_PFC_random all_Test_GC_TO_PFC_random
