...
function supp_s08_decode_GC_B(arrayInput)
...
possPFCCoords = {'L_SP_PFC','R_SP_PFC','L_Inf_PFC','R_Inf_PFC'}%,'PFC coords
possSensoryCoords = {'L_FFA','R_FFA','L_Occ','R_Occ'}; %sensory coords

%Define parameters
load('PLIST.mat') %list of participants
...
for thisCVRep = 1:nCVReps
    ...
    for PFCIdx = 1:length(possPFCCoords)
        ...
        for sensoryIdx = 1:length(possSensoryCoords)

            ROIcount = ROIcount + 1;
            useSensoryCoord = possSensoryCoords{sensoryIdx};

            loadname =  ['GC_rmline_' (PName) '_' (thisPhase) '_CVmethod_thisRep' num2str(thisCVRep) ...
                '_NoFilt_wBC_zscore=_1_bw_' num2str(usePFCCoord) ...
                '_and_' num2str(useSensoryCoord) '_by_TstatOverTime_usingTrainProp_' num2str(trainProp) '_and_' ...
                num2str(timeWindow(1)) '_to_' num2str(timeWindow(2)) 'ms.mat'];

            savename_Results = ['results_GC_fromPFC_Backto_Sensory_rmline_' (PName) '_' (thisPhase)  ...
                '_CVmethod_10REPS_GC_' num2str(maxFreq) 'Hz_MultiROI_' ...
    ...
    all_FACES_Sensory_FROM_PFC_allReps(thisCVRep,:,:,:) = [all_Train_GC_FROM_PFC_faces;all_Test_GC_FROM_PFC_faces];
    all_RANDOM_Sensory_FROM_PFC_allReps(thisCVRep,:,:,:) = [all_Train_GC_FROM_PFC_rand;all_Test_GC_FROM_PFC_rand];
